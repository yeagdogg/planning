...
def build_netcalc(ctx: Ctx):
    ...
    # The three reasonability bounds live on the SOLVER sheet as editable cells,
    # which a light workbook does not have (D109). Net Delivery is a keeper
    # either way, so its flag column falls back to the same numbers baked as
    # literals — the values those cells are seeded with. The wording of the
    # flags does not change; only whether the reader can retune the threshold.
    if ctx.has(SHEETS.SOLVER):
        max_rate, min_share, max_mod = "nr_SolvMaxRate", "nr_SolvMinShare", "nr_SolvMaxMod"
    else:
        max_rate = repr(float(ctx.cfg.solver_max_rate))
        min_share = repr(float(ctx.cfg.solver_min_post_share))
        max_mod = repr(float(SOLVER_MAX_MOD_STEP))
    ...
    for i in range(nd + 1):
        ...
        formula(ws, f"M{rr}",
                ...
                + "&" + num(f"$F{rr}", f"ABS($F{rr})>{max_rate}", "RATE-BOUND")
                + "&" + num(f"$L{rr}", f"$L{rr}<{min_share}", "THIN-SHARE")
                + "&" + num(f"$S{rr}", f"ABS($S{rr})>{max_mod}", "MODSTEP-BOUND")
...
def build_net_delivery(ctx: Ctx):
    ...
    nav_bar(ws, 3, 1, [s for s in ["Control", "State Summary", "Rate Log", "Solver",
                                   "Checks"] if ctx.has(s)], step=2)
    ...
    # the third clause warns that a Scenarios lever is set; a light workbook has
    # no Scenarios tab, so there is no lever to warn about (D109)
    scen_note = ('IF(COUNT(sc_dnet)>0,"NOTE: a Scenarios delta-net lever is set; '
                 'scenario levers do not affect this tab.","")'
                 if ctx.has(SHEETS.SCENARIOS) else '""')
    formula(ws, "A6",
