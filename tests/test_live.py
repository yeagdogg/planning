...
# ---------------------------------------------------------------------------
# live: the TARGETS file computes in Excel exactly what the engine says (W4a)
# ---------------------------------------------------------------------------

@pytest.fixture(scope="module")
def targets(tmp_path_factory):
    """A freshly built targets file, opened once in Excel. Separate from the
    fleet fixture above: this file is generated by the app, not the fleet
    builder, and the point is to prove ITS formulas."""
    ...
    path = targetbook.build_targets_file(scn, out_dir=tmp)
    lb = LiveBook(path, tmp / "live.xlsx")
    yield lb, scn
    lb.close()
    shutdown_excel()


def _first_live_row(scn):
    """The first combo on the first BU tab — the row every assertion below
    drives. Returns (sheet name, excel row, lr_row dict)."""
    from app import targetbook
    row = next(r for r in scn.lr_rows if r.get("bu") and r.get("state"))
    return row


@live_only
def test_targets_file_computes_the_engine_in_excel(targets):
    """The claim the whole file rests on: type a net target into Excel and
    the cell shows what run_bridge would return — not an approximation of
    it. Then clear it and the logged program comes back."""
    ...
    from tools.live import scan_errors_live
    lb, scn = targets
    row = _first_live_row(scn)
    bu, state = row["bu"], row["state"]
    sheet = next(s for s in lb.sheetnames
                 if s not in (targetbook.COVER, targetbook.FACTORS)
                 and lb[s]["Q1"].value == bu)
    r = targetbook.FIRST_ROW + [x["state"] for x in scn.lr_rows
                                if x.get("bu") == bu].index(state)
    ci = compute.combo_inputs(scn, row)

    assert scan_errors_live(lb) == []

    # a typed P target: both years move, P+1 CARRYING the same target
    wb = lb.apply({(sheet, f"H{r}"): 0.06})
    want = engine.run_bridge(
        scn.plan_year, replace(ci, net_sel_p=0.06, net_sel_p1=0.06),
        "monthly")
    assert wb[sheet][f"J{r}"].value == pytest.approx(want.cy_lr_p, abs=1e-9)
    assert wb[sheet][f"K{r}"].value == pytest.approx(want.cy_lr_p1, abs=1e-9)

    # an explicit P+1 target overrides the carry
    wb = lb.apply({(sheet, f"H{r}"): 0.06, (sheet, f"I{r}"): 0.02})
    want2 = engine.run_bridge(
        scn.plan_year, replace(ci, net_sel_p=0.06, net_sel_p1=0.02),
        "monthly")
    assert wb[sheet][f"K{r}"].value == pytest.approx(want2.cy_lr_p1, abs=1e-9)

    # the suggested change ties the engine's own closed form
    wb = lb.apply({(sheet, f"H{r}"): 0.06, (sheet, f"L{r}"): "7/1"})
    import datetime as dt
    sugg, _c = engine.suggest_net_rate(
        scn.plan_year, replace(ci, net_sel_p=0.06, net_sel_p1=0.06),
        dt.date(scn.plan_year, 7, 1))
    assert wb[sheet][f"M{r}"].value == pytest.approx(sugg, abs=1e-9)

    # blank clears net mode: back to the logged program, not to zero
    wb = lb.apply({(sheet, f"H{r}"): "", (sheet, f"I{r}"): "",
                   (sheet, f"L{r}"): ""})
    prog = engine.program_basis_plan_lr(scn.plan_year, ci)
    assert wb[sheet][f"J{r}"].value == pytest.approx(prog[0], abs=1e-9)
    assert wb[sheet][f"K{r}"].value == pytest.approx(prog[1], abs=1e-9)
    assert wb[sheet][f"M{r}"].value in ("", None)
    assert scan_errors_live(lb) == []
    lb.restore()
